...
class FormatConverter {
    ...
    static DetectFormat(content, useClipboardHints := true) {
        ...
        if (content == "" || content := unset) {
            return "plaintext"
        }
        
        ; Check if content matches clipboard and use clipboard format detection
        if (useClipboardHints && content == A_Clipboard) {
            clipFormat := this.DetectClipboardFormat()
            if (clipFormat != "unknown") {
                return clipFormat
            }
            ; Fall through to text analysis if clipboard format detection fails
        }
    ...
    /**
     * @description Detects the format based on available clipboard formats using existing Clipboard class
     * @returns {String} Detected format: "rtf", "html", "unicode", "text", or "unknown"
     */
    static DetectClipboardFormat() {
        try {
            ; Use existing Clipboard class methods to check formats
            
            ; Check for RTF format first (most specific)
            if (Clipboard.IsFormatAvailable(Clipboard.RegisterFormat.RTF)) {
                return "rtf"
            }
            
            ; Check for HTML format
            if (Clipboard.IsFormatAvailable(Clipboard.RegisterFormat.HTML)) {
                return "html"
            }
            
            ; Check for CSV format
            if (Clipboard.IsFormatAvailable(Clipboard.RegisterFormat.CSV)) {
                return "csv"
            }
            
            ; Check for TSV format
            if (Clipboard.IsFormatAvailable(Clipboard.RegisterFormat.TSV)) {
                return "tsv"
            }
            ...
            if (Clipboard.IsFormatAvailable(CF_UNICODETEXT)) {
                ; Try to get Unicode text and analyze it
                unicodeText := Clipboard.GetUnicode()
                if (unicodeText) {
                    ; Quick check for common markup patterns
                    if (this.VerifyRTF(unicodeText).isRTF) {
                        return "rtf"
                    }
                    if (this.VerifyHTML(unicodeText).isHTML) {
                        return "html"
                    }
                    if (RegExMatch(unicodeText, "^#|^\*\*|^- ")) {
                        return "markdown"
                    }
                }
                return "unicode"
            ...
            static CF_TEXT := 1
            if (Clipboard.IsFormatAvailable(CF_TEXT)) {
                return "text"
            }
	...
	static VerifyRTF(content) {
		arrRTFChecks := []
		quickRTF := [
			; Standard RTF headers with variations
			"^{\\rtf1",            ; Standard RTF start at beginning of string
			"^[ \t]*{\\rtf1",      ; RTF with leading whitespace
			"{\\rtf1\\ansi",       ; Common ANSI RTF header
			"{\\rtf1\\ansicpg",    ; RTF with codepage specification
			"{\\rtf1\\mac",        ; Mac RTF variant
			"{\\rtf1\\pc",         ; PC RTF variant
			"{\\rtf1\\pca",        ; PCA RTF variant
			"{\\rtf1\\unix",       ; UNIX RTF variant
			
			; RTF version variations
			"{\\rtf\\",            ; Generic RTF marker (less common)
			"{\\rtf[0-9]",         ; Any RTF version number
			
			; Common RTF structure elements that strongly indicate RTF format
			"\\fonttbl",           ; Font table definition
			"\\colortbl",          ; Color table definition
			"\\stylesheet",        ; Style sheet definition
			"\\pard",              ; Paragraph definition (very common in RTF)
			"\\par ",              ; Paragraph mark
			"\\fs[0-9]",           ; Font size specification
			
			; Specific RTF version headers (for thoroughness)
			"{\\rtf1\\ansi\\ansicpg[0-9]+", ; ANSI with specific codepage
			"{\\rtf1\\ansi\\deff",          ; ANSI with default font
			
			; Simpler indicators (use with other checks)
			"\\viewkind[0-9]",     ; View kind specification
			"\\uc[0-9]"            ; Unicode specification
		]

		; Check for these patterns but only after validating other indicators
		secondaryRTF := [
			"\\f[0-9]",            ; Font reference
			"\\cf[0-9]",           ; Color reference
			"\\margl[0-9]",        ; Margin left
			"\\margr[0-9]"         ; Margin right
		]
		...
		; if (content ~= "^{\\rtf1" || content ~= "{\\rtf1" || content ~= "{\\rtf" || content ~= "{\\rtf1"){
		; 	return {isRTF: false, content: content}
		; }
		; if !(content ~= 'rtf1') || !(content ~= '\\rtf1') || !(content ~= 'rtf') || !(content ~= '\\rtf') {
		; 	return {isRTF: false, content: content}
		; }
		...
		; [arrRTFChecks, quickRTF, secondaryRTF, rtfChecks].Flat()
		matchCount := 0
	...
	static toRTF(text:=''){
		...
		if (RegExMatch(text, "^{\rtf1")) { ; Already RTF
			return text
		} else if (RegExMatch(text, "^<!DOCTYPE html|^<html")) { ; HTML
			return FormatConverter.HTMLToRTF(text)
		} else if (RegExMatch(text, "^#|^\*\*|^- ")) { ; Markdown
			return FormatConverter.MarkdownToRTF(text)
		} else { ; Plain text
	...
	; ---------------------------------------------------------------------------
	;? Enhanced version, didn't test, but improved version above
	; /**
	;  * @description Enhanced RTF processing with proper formatting maintenance
	;  * @param {String} rtf The RTF content to process
	;  * @param {Boolean} standardize Whether to force standardization
...
class String2 {
    ...
    static Initialize() {
		...
		__ObjDefineProp(String.Prototype,"MarkdownToRTF", {Call: (self) => FormatConverter.MarkdownToRTF(self)})
		__ObjDefineProp(String.Prototype, "HTMLToRTF", {Call: (self) => FormatConverter.HTMLToRTF(self)})
		__ObjDefineProp(String.Prototype, "_isRTFContent", {Call: (self) => FormatConverter.VerifyRTF(!IsSet(content)?content:=this:content).isRTF})
	...
	static Send(input?) {
		...
		Type(this) == 'String' ? Clipboard.Send(this) : Clipboard.Send(input)
	}
	...
	static RTF(text:=''){
		...
		; Plain text - convert to RTF
		return FormatConverter.ToRTF(text)
	...
	/**
	 * @description Converts a string to an array using various separators
	 * @param {String} [separator] Separator pattern to use: "lines" (default), "csv", "tsv", "words", "chars", or a custom separator
	 * @returns {Array} Array containing split elements
	 */
	static ToArray(separator := "lines") {
		; Handle predefined separator patterns
		switch (separator) {
			case "lines":
				return StrSplit(this, "`n", "`r")
			case "csv":
				return StrSplit(this, ",", " `t`r`n")
			case "tsv":
				return StrSplit(this, "`t", " `r`n")
			case "words":
				return StrSplit(this, " `t", "`r`n")
			...
			default:
				...
				return StrSplit(this, separator)
		}
	}
...
AppendFile(whichFile, text) {
	FileAppend(text, whichFile, "UTF-8-RAW")
}
...
findtheglobal(display:= false){
	...
	SM()
	Clipboard.BackupAndClear(&cBak)
	Send(keys.SelectAll keys.copy)
	Sleep(A_Delay)
	...
	display ? Infos(newtheText) : Outputdebug(newtheText)
